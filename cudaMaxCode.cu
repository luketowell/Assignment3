...
__global__ void findMaxY (float *Y, int dataPoints, float *devMaxY) {
    __shared__ float devY_shared[numElements]; 
    int my_i =  threadIdx.x;
    devY_shared[my_i] = Y[my_i];
    while (dataPoints/2 > 1) {
         if(devY_shared[my_i]<devY_shared[my_i+(dataPoints/2)]){
              devY_shared[my_i] = devY_shared[my_i + (dataPoints/2)];
            __syncthreads();
	 }	
        dataPoints = dataPoints / 2; 
	__syncthreads();
    }  
    devY_shared[0] = devY_shared[0]<devY_shared[1] ? devY_shared[1] : devY_shared[0];
    devMaxY[0] = devY_shared[0];	
}
...
{   
   ...
   if (numGPU >= 1) {
      ...
      cudaMemcpy(devFx, Y, dataPoints*sizeof(float), cudaMemcpyHostToDevice);
      // Run findMaxY Kernel
      findMaxY<<<blocks, threads>>>(devFx, dataPoints, devMaxY);     
